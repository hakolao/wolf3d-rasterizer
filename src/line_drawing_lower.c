...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/09/14 17:51:04 by veilo             #+#    #+#             */
/*   Updated: 2020/09/14 17:51:06 by veilo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	plot_line_low(t_ir p1, t_ir p2, uint32_t color, t_wolf3d *app)
{
	...
	while (++xy[0] <= p2.a)
	{
		plot_pixel_lower(app, app->main_window->rbuffer, xy, color);
		if (dyi[0] > 0)
		{
			xy[1] += dyi[1];
			dyi[0] -= 2 * dxy[0];
...
static void	plot_line_high(t_ir p1, t_ir p2, uint32_t color, t_wolf3d *app)
{
	...
	while (++xy[1] <= p2.b)
	{
		plot_pixel_lower_high(app, app->main_window->rbuffer, xy, color);
		if (dxi[0] > 0)
		{
			xy[0] += dxi[1];
			dxi[0] -= 2 * dxy[1];
