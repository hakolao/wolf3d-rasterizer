...
/*   Updated: 2020/10/15 16:42:14 by ohakola          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			move_player(t_wolf3d *app, t_move dir)
{
	...
	ml_matrix4_rotation_y(ml_rad(app->player.rot_x), rotation_x);
	ml_matrix4_mul_vec3(rotation_x, app->player.forward, forward);
	ml_matrix4_mul_vec3(rotation_x, app->player.sideways, sideways);
	if (dir == move_forward)
	{
	...
	ml_matrix4_translation(app->player.pos[0], app->player.pos[1],
		app->player.pos[2], app->player.world_translation);
}
