...
static void		render_triangle(t_wolf3d *app, uint32_t *buffer,
								void (*render_f)(uint32_t *buf,
												uint32_t *dim,
												t_triangle *tri),
								t_triangle *triangle)
{
		...
		&app->active_scene->main_camera->viewplanes[0], clipped_triangles))
	{
		...
		render_f(buffer, dimensions, &clipped_triangles[0]);
		render_f(buffer, dimensions, &clipped_triangles[1]);
	}
	else
	{
		screen_intersection(app, triangle);
		render_f(buffer, dimensions, triangle);
	}
}
...
static void		rasterize_work(void *params)
{
	...
	work = params;
	render_triangle(work->app, work->app->window->framebuffer,
		l3d_triangle_raster, work->triangle);
	free(work);
}
...
static void		zbuffer_work(void *params)
{
	...
	work = params;
	render_triangle(work->app, work->app->window->zbuffer,
		l3d_triangle_set_zbuffer, work->triangle);
	free(work);
}
