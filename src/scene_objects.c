...
t_3d_object		**create_test_scene_objects(int32_t *object_count)
{
	...
	*object_count = 1;
	...
	ml_vector3_copy((t_vec3){WIDTH, 50, 200}, objects[0]->mesh_vertices[0].position);
	ml_vector3_copy((t_vec3){WIDTH, -WIDTH / 2 + 5, 250}, objects[0]->mesh_vertices[1].position);
	ml_vector3_copy((t_vec3){WIDTH, -WIDTH / 2 + 40, 300}, objects[0]->mesh_vertices[2].position);
